...
ComBat <- function(expression_xls, sample_info_file, type='txt', write=T, covariates='all', par.prior=T, filter=F, skip=0, prior.plots=T){
	...
	if (skip>0){
              geneinfo <- as.matrix(dat2[,1:skip])
              dat <- dat[,-c(1:skip)]}
        else{geneinfo=NULL}
        #print(geneinfo[1:4])
        #print(dat[1:2,])
	...
	if(write){
		output_file <- paste('Adjusted',expression_xls,sep='_')
                 #print(geneinfo[1:2])
		...
		#suppressWarnings(write.table(cbind(geneinfo,formatC(as.matrix(bayesdata), format = "f")), file=output_file, sep="\t", quote=F,row.names=F,col.names=F,append=T))
                ProbeID<-geneinfo
		outdata <- cbind(ProbeID, bayesdata); write.table(outdata,file=output_file,row.names=F,sep="\t",quote=F)
