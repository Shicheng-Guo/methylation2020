...
# update 11/18/2015, add race into the model, do the mete-regression and subgroup analysis




# pipeline meta analysis for CHD13 methylation diagnostic role in NSCLC. 
setwd("/home/sguo/Dropbox/Project/methylation/meta/FHIT")
setwd("C:\\Users\\shicheng\\Dropbox\\Project\\methylation\\meta\\FHIT\\result")
...
subgroup<-c("sampletype","age","stage1","stage2","genderatio","methods","aim","Multipletarget","controldesign","Ad2Sc","primer","race")
colnames(apc)
...
subdata<-apc[! is.na(apc[,i]), ]
subdata
less<-which(subdata[,i] =="Asian")
more<-which(subdata[,i] =="European")
subdata[array(less),i]<-"Asian"
subdata[array(more),i]<-"European"
meta2 <- metabin(event.e, n.e, event.c, n.c,data=subdata,studlab=author3,sm="OR", method="I",method.tau="DL",byvar=subdata[,i],print.byvar=F,label.e="NSCLC",label.c="Normal")
summary(meta2)
filename<-paste(colnames(subdata)[i],"_stratify.pdf",sep="")
pdf(filename)
