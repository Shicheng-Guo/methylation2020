...
statistic for venn graph. Overlap DMR is quite few. 
```{r}
setwd("C:\\Users\\shicheng\\Dropbox\\Project\\esophageal_carcinoma\\jingde\\venn")
file=list.files(pattern="*.venn")
Loc<-c()
for(i in 1:length(file)){
  loc<-read.table(file[i],sep="\t",as.is=T)
  Loc<-c(Loc,loc[,1])
}
barplot(table(table(as.character(Loc))),col="blue",ylim=c(0,15000))
```