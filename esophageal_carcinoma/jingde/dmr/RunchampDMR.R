DMR.CHAMP<-function(){
  ...
  # write.table(Rlt$dmr$myDmrs$table,file=outputFile3,col.names=NA,row.names=T,sep="\t",quote=F)
  # ProbeLasso Method (need mvp)
  result2=champ.DMR(betaNorm=myNorm$beta,design=myLoad$pd$Sample_Group,maxGap=300,
                    cutoff=0.1,minProbes=3,smooth=TRUE,smoothFunction=loessByCluster,
                    useWeights=FALSE,permutations=NULL,B=10,pickCutoff=FALSE,
                    pickCutoffQ=0.99,nullMethod="bootstrap",verbose=TRUE,cores=3,arraytype="450K",
                    method = "ProbeLasso",resultsFile=mylimma,meanLassoRadius=375,minSigProbesLasso=3,minDmrSep=1000,
                    minDmrSize=50,adjPvalProbe=1,adjPvalDmr=0.05,pData=myLoad$pd)
...
for(i in 1:ncol(R)){
  ...
  Rlt<-DMR.CHAMP()
  outputFile3=paste(colnames(R)[i],"DMR.txt",sep=".")
  outputFile4=paste(colnames(R)[i],"DMR.Probes.txt",sep=".")
  write.table(Rlt$dmr$myDmrs,file=outputFile3,col.names=NA,row.names=T,sep="\t",quote=F)
  write.table(Rlt$dmr$myDmrProbes,file=outputFile4,col.names=NA,row.names=T,sep="\t",quote=F)
...
# DMR statistic
setwd("/home/shg047/bak/zh/data/zhu2016/dataset/idat")
file=list.files(pattern="*DMR.txt")
for(i in 1:length(file)){
  data=read.table(file[i],head=T,sep="\t")
  output=data.frame(data$dmrChrom,data$dmrStart,data$dmrEnd,data$dmrP)
  write.table(output,file=paste(file[i],".bed",sep=""),sep="\t",col.names=F,row.names = F,quote=F)
}

multiIntersectBed -i 5FU.DMR.txt.bed ChemSum.DMR.txt.bed Ci.DMR.txt.bed Dt.DMR.txt.bed Mi.DMR.txt.bed Nd.DMR.txt.bed Pa.DMR.txt.bed Radiotherapy.DMR.txt.bed Vb.DMR.txt.bed
