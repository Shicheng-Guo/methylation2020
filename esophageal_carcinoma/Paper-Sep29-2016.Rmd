...
setwd("f:/Esphageal Cancer/2016.9.2/")
#process the main data file
data           = read_excel("methylation.xlsx",sheet = 2)
data           = as.data.frame(data)
data           = data[,-4]
data[,-c(1:10)]= apply(data[,-c(1:10)],2,function(x) {return(as.numeric(x))})
data$Target    = sapply(data$Target, function(x){return(strsplit(as.character(x),"_")[[1]][1])})
# Process the info file
info           = read_excel("methylation.xlsx",sheet = 4)
info           = as.data.frame(info)
info           = info[,-5]
info[,7:8]     = apply(info[,7:8],2,function(x) {return(as.numeric(x))})
info$Target    = sapply(info$Target, function(x){return(strsplit(as.character(x),"_")[[1]][1])})
```
...
seq.data = which(data$Target =="LINE-1")
seq.info = which(info$Target =="LINE-1")
pos.data = data[seq.data,]
pos.info = info[seq.info,]
data = data[-seq.data,]
info = info[-seq.info,]
#Split the negative control datasets
seq.data = which(data$Target =="chrM")
seq.info = which(info$Target =="chrM")
neg.data = data[seq.data,]
neg.info = info[seq.info,]
data = data[-seq.data,]
info = info[-seq.info,]
#randomly select seven of the CpGsites, and do the analysis.
Total.CpGsites = unique(info$`POS.In Genome`)
Selected.CpGsites = sample(Total.CpGsites,7)
temp = data[match(Selected.CpGsites, data$`POS.In Genome`), ]
Type = sapply(colnames(temp)[-c(1:10)], function(x){ return(substr(x,1,1)) })
Type = ifelse(Type == "N", "Control", "Cancer")
data_plot = as.data.frame(t(temp[,-c(1:10)]))
Selected.CpGsites = paste(temp$Target[match(Selected.CpGsites, temp$`POS.In Genome`)], Selected.CpGsites, sep="-")
colnames(data_plot) = Selected.CpGsites
data_plot = data.frame(Type, data_plot)
data_table.random = data_plot
Type = rep(data_plot$Type,7)
Methylation = c(t(data_plot[,2]),t(data_plot[,3]), t(data_plot[,4]), t(data_plot[,5]), t(data_plot[,6]),
                t(data_plot[,7]),t(data_plot[,8]))
CpG = c(rep(colnames(data_plot)[2], dim(data_plot)[1]),rep(colnames(data_plot)[3], dim(data_plot)[1]),
        rep(colnames(data_plot)[4], dim(data_plot)[1]),rep(colnames(data_plot)[5], dim(data_plot)[1]),
        rep(colnames(data_plot)[6], dim(data_plot)[1]),rep(colnames(data_plot)[7], dim(data_plot)[1]),
        rep(colnames(data_plot)[8], dim(data_plot)[1]))
data_plot.random = data.frame(Type, Methylation, CpG)
#randomly select one of the positive CpGsites, and do the analysis.
Total.CpGsites = unique(pos.info$`POS.In Genome`)
Selected.CpGsites.pos = sample(Total.CpGsites,1)
temp = pos.data[match(Selected.CpGsites.pos, pos.data$`POS.In Genome`), ]
Type = sapply(colnames(temp)[-c(1:10)], function(x){ return(substr(x,1,1)) })
Type = ifelse(Type == "N", "Control", "Cancer")
data_plot = as.data.frame(t(temp[,-c(1:10)]))
Selected.CpGsites.pos = paste(temp$Target[match(Selected.CpGsites.pos, temp$`POS.In Genome`)], Selected.CpGsites.pos, sep="-")
colnames(data_plot) = Selected.CpGsites.pos
data_plot = data.frame(Type, data_plot)
data_table.pos = data_plot
Type = rep(data_plot$Type,1)
Methylation = c(t(data_plot[,2]))
CpG = c(rep(colnames(data_plot)[2], dim(data_plot)[1]))
pos.data_plot = data.frame(Type, Methylation, CpG)
#randomly select one of the negative CpGsites, and do the analysis.
Total.CpGsites = unique(neg.info$`POS.In Genome`)
Selected.CpGsites.neg = sample(Total.CpGsites,1)
temp = neg.data[match(Selected.CpGsites.neg, neg.data$`POS.In Genome`), ]
Type = sapply(colnames(temp)[-c(1:10)], function(x){ return(substr(x,1,1)) })
Type = ifelse(Type == "N", "Control", "Cancer")
data_plot = as.data.frame(t(temp[,-c(1:10)]))
Selected.CpGsites.neg = paste(temp$Target[match(Selected.CpGsites.neg, temp$`POS.In Genome`)], Selected.CpGsites.neg, sep="-")
colnames(data_plot) = Selected.CpGsites.neg
data_plot = data.frame(Type, data_plot)
data_table.neg = data_plot
Type = rep(data_plot$Type,1)
Methylation = c(t(data_plot[,2]))
CpG = c(rep(colnames(data_plot)[2], dim(data_plot)[1]))
neg.data_plot = data.frame(Type, Methylation, CpG)
