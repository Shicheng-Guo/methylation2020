setwd("/home/gsc/Dropbox/Project/FuntionalCulster/SenResPlot")
gene<-read.table("genes.txt",head=F,sep="\t")
column.list<-c(0,cumsum(gene[,1])) # the first 6 column are fixed for ped file
head(column.list)
...
xlist<-cbind(gene,list)
write.table(xlist,file="column.list.txt2",col.names=F,row.names=F,sep="\t",quote=F)
list<-read.table("column.list.txt2",sep="\t")
head(list)
i<-j<-numeric()
fpca.rlt<-list()
