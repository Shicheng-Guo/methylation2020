...
#my @sample_names = ("NC-P-1","NC-P-2","NC-P-3","NC-P-5","NC-P-6","NC-P-7","NC-P-8","NC-P-9","NC-P-13","NC-P-14","NC-P-15","NC-P-16","NC-P-17","NC-P-18","NC-P-19","NC-P-20","NC-P-21","NC-P-22","NC-P-23","NC-P-25","NC-P-26","NC-P-27","NC-P-29","NC-P-30");
my %hap_count_matrix;
...
#		print "#$sample_name\n";
		...
		foreach my $word_size (keys(%k_mer_counts)){
			...
			$norm_factor+=$weight;
#			print "word_size=$word_size\tmh_fraction=$mh_fraction\tmc_hap_load=$mc_hap_load\tnorm_factor=$norm_factor\n"; 
		}
		next if(!$norm_factor);
...
foreach my $probeID (keys(%mch_load_matrix)){
	...
	foreach my $sample_name(@sample_list){
		$mch_load_matrix{$probeID}->{$sample_name}=0 if(!$mch_load_matrix{$probeID}->{$sample_name});
