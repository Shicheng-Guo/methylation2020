...
### Data: WGBS, RRBS (in genome-miner) are saved in **/media/Home_Raid1/shg047/NAS1/monod/mhl/0530**
...
```
GSI calculation procedure is quite time-consuming (20 min for 147,888), I have already created the GSI, therefore, if you don't change the paramteres or method, you can use GSI result from me. it is save in **gsirlt.mhl.may30.RData**. As the below, you can find the format of GSI: MHB, Tissue Group, GSI, max-MHL-in-Reference, MHL for each tissue.
```{r checkgsirlt}
...
rlt1<-prediction1(Data,bio,tt=0.3)  
rlt1$prediction
...
```{r}
rlt2<-prediction1(data,bio,tt=0.3)              # tissue=c(0.3-0.6)
rlt2$prediction
```
...
rlt3<-ZscorePredictionPlasma1(nncp,ncp,bio,tt=0.001)    # tt is threshold to binary the MHL matrix, ccp: colon cancer plasma, ncp: normal plasma trainning dataset
rlt1$score[1:10,1:3]
rlt2$score[1:10,1:3]
rlt3$score[1:10,1:3]
...
heatmaprlt<-HeatMap(rlt3$score)
heatmaprlt<-HeatMap(t(rlt3$score))
dev.off()
```
...
rlt1<-ZscorePredictionPlasma2(nncp,ncp,bio,tt=i)   
rlt2<-ZscorePredictionPlasma2(ccp,ncp,bio,tt=i)
prin1<-sum(unlist(apply(rlt1$score,2,function(x) rownames(rlt1$score)[which.max(x)]))=="Colon")
