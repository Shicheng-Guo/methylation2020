...
rlt<-heatmap.2(data.matrix(SigDiffMHBANOVA),col=col,trace="none",density.info="none",Colv=T,Rowv=T,key=T,keysize=1,cexCol=0.65,cexRow=0.35)
dev.off()

data=data.matrix(SigDiffMHBANOVA)
dim(data)
d<-dist(data)
fit<-hclust(d)
plot(as.dendrogram(fit),horiz=T)

data=read.table("Table.GSI.layer.mhl.WGBS.Remove.H1.WBC.rlt.txt")
rlt=data.frame(region=rownames(fn),group=gmaxgroup,GSI=gsi)
write.table(rlt,file="Table.GSI.layer.mhl.WGBS.Remove.H1.WBC.rlt.txt",col.names=T,row.names=T,quote=F,sep="\t")
rlt$rowInd
rlt$rowDendrogram
colnames(SigDiffMHBANOVA)[rlt$colInd]
