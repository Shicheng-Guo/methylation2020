...
setwd("/home/shg047/oasis/monod/mixHap")
...
data<-data1[,c(grep("PC-P",colnames(data1)),grep("PC-T",colnames(data1)),grep("NC-P",colnames(data1)), grep("N37-Pancreas|PA-|STL",colnames(data1)))]
colnames(data)
target1.colon<-which(apply(data,1,function(x) mean(x[11:15],na.rm=T)>0.5 && mean(x[16:78],na.rm=T)<0.05))
length(target1.colon)
data=data[target1.colon,]
rownames(data)<-rownames(data1)[target1.colon]
write.table(data,file="pancreatic.data.plsma.hypoall.txt",sep="\t",quote=F,col.names=NA,row.names=T)


colon.data=data
cp<-tt<-np<-nt<-c()
for(i in 1:length(target1.colon)){
  cp<-c(cp,mean(as.numeric(data[i,1:10]),na.rm=T))
  tt<-c(tt,mean(as.numeric(data[i,11:15]),na.rm=T))
  np<-c(np,mean(as.numeric(data[i,16:41]),na.rm=T))
  nt<-c(nt,mean(as.numeric(data[i,42:78]),na.rm=T))
...
write.table(rownamedp,file="pancreatic.mixhap.mhl.in.plsma.hypoall.bed",sep="\t",quote=F)

write.table(dp,file="pancreatic.mixhap.mhl.in.plsma.hypoall.txt",sep="\t",quote=F)
dp2<-as.numeric(dp)
...
myData
myData$sd<-c(0.10,0.01,0.01,0.11)


# Plot one standard error (standard error of the mean/SEM)
pdf("pancreatic.barplot.ggplot2.hypoall.pdf", height = 5, width = 4)
