...
# Version 1.3
# Update: May/5/2017
# Input: WGBS,CT, LC, CC and NC MHL matrix and tissue biomarker and then identify cancer biomarker and then do prediction(t1>0.01 and t2>0.13)
...
input<-DATA[match(subset((gsirlt),group=="CT")[,1],rownames(DATA)),grep("NT",colnames(DATA))]
  ...
  CCPF(cc1,threshold1=0.01,threshold2=0.13,zz="CCPF")
  LCPF(cc2,threshold1=0.01,threshold2=0.13,zz="LCPF")
  
  zz<-0
  for(threshold2 in seq(0,0.2,0.01)){
    zz=zz+1
    threshold1       # cancer biomarker threshold in normal tissues
    threshold2       # This threshold can not like the threshold of Figure 5, since there will have remove WBC background
