...
# CT markers can optimized with or without the normal tissue, the result don't have big changes
library("gplots")
...
heatmapCounts<-function(CCP,LCP,NCP,output){
  dim(normal)
  dim(CCP)
  dim(LCP)
...
################## Prediction #############################
###########################################################
for(threshold2 in seq(0,0.4,0.01)){
threshold2       # This threshold is not like the threshold of Figure 5, since there will have remove WBC background
...
xx1<-mean(cc1[3,]-cc[3,])
xx2<-mean(cc2[3,]-cc[3,])
outputfile1<-paste("heatmap-count",threshold1,threshold1,sep="-")
heatmapCounts(cc1,cc2,cc,outputfile1)
...
beeswarm(input,col = 2:12,pch=16,method="center",ylab="Z-Score",main="Colon cancer plamsa")
dev.off()
...
jpeg("ccp.beeswarm.jpg")
input<-list(Brain=Zmax[1,],Colon=Zmax[2,],Intestine=Zmax[3,],Kidney=Zmax[4,],Liver=Zmax[5,],Lung=Zmax[6,],Pancreas=Zmax[7,],Spleen=Zmax[8,],stomach=Zmax[9,],WBC=Zmax[10,],CT=Zmax[11,])
beeswarm(input,col = 2:12,pch=16,method="center",ylab="Z-Score",main="Lung cancer plamsa")
dev.off()
...
mydata<-mydata[na.omit(match(rownames(bio3),rownames(mydata))),]
