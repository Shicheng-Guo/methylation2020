...
request = 'http://api.basespace.illumina.com/v1pre3/projects/%s/samples?access_token=%s' %(ProjectID,AccessToken)
...
for index in range(noffsets):
	offset = 1000*index
	request = 'http://api.basespace.illumina.com/v1pre3/projects/%s/samples?access_token=%s&limit=1000&Offset=%s' %(ProjectID,AccessToken,offset) 
	json_obj = restrequest(request)
