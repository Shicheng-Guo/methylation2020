...
my $mean_error_rate = $ARGV[4] ? $ARGV[4] : 0.1;
my $min_evidence = $ARGV[5] ? $ARGV[5] : 0.3;
...
sub main{
	...
	foreach my $probeID (keys(%hapTable)){
		next if($probeID =~ /chr[XYM]/); #ignore sex chromosomes
		...
		my ($haps_assigned_to_A, $haps_assigned_to_B,$haps_unassigned, $score_A, $score_B) = assign_haps_by_similarity(\%hapClusterA, \%hapClusterB, \%hapClusterMix);
	...
	$total_log_score_to_A = sprintf("%.0f", $total_log_score_to_A);
	$total_log_score_to_B = sprintf("%.0f", $total_log_score_to_B);
...
sub assign_haps_by_similarity(){
	...
	my $total_hap_A = ${$h_cluster_A}{"total_hap_count"};
	my $total_hap_B = ${$h_cluster_B}{"total_hap_count"};
	...
	foreach my $hap (keys(%{${$h_cluster_Mix}{"hapCounts"}})){
		...
		if(${$h_cluster_Mix}{"z_score_in_ref_A"}->{$hap} - ${$h_cluster_Mix}{"z_score_in_ref_B"}->{$hap} > 0.5){
			$abundance_in_A = calcSmoothAbundance(0,$total_hap_A); #If the difference in Z_score is greater than 0.5, do not count.
		}elsif(${$h_cluster_Mix}{"z_score_in_ref_B"}->{$hap} - ${$h_cluster_Mix}{"z_score_in_ref_A"}->{$hap} > 0.5){
			$abundance_in_B = calcSmoothAbundance(0,$total_hap_B); #If the difference in Z_score is greater than 0.5, do not count.
		}
		#print "$hap\t$abundance, RefA: $abundance_in_A RefB: $abundance_in_B\n";
		my $total_evidence = $abundance_in_A + $abundance_in_B;
		if($total_evidence > $min_evidence){ #evidence should be at least 0.5? KZ had 0.001
			my $To_A=${$h_cluster_Mix}{"hapCounts"}->{$hap}*($abundance_in_A/$total_evidence);
			my $To_B=${$h_cluster_Mix}{"hapCounts"}->{$hap}*($abundance_in_B/$total_evidence);
			$haps_assigned_to_A+=$To_A;
			$haps_assigned_to_B+=$To_B;
			if($abundance_in_A != 0 && $abundance_in_B != 0){
				$score_A += $To_A*log($abundance_in_A/$total_evidence);
				$score_B += $To_B*log($abundance_in_B/$total_evidence);
			}
			# if one have posterior of 0, the other will get a score of 0, so do nothing
			#else{
...
sub find_all_haplotype_cluster(){
	...
	}while($remainingHaps > 0 && $totalHaps > 0 && $hapsCount_in_cluster > 0);
...
sub load_data_files{
	...
	my $hap_number=0;
	open(INFILE, "$ref_hap_file_A.haploInfo.txt")||die("Error in opening file $ref_hap_file_A\n");
	...
	$hap_number=0;
	open(INFILE, "$ref_hap_file_B.haploInfo.txt")||die("Error in opening file $ref_hap_file_B\n");
	while(my $line = <INFILE>){
		chomp($line);
		my @fields = split(/\t/, $line);
		next if(scalar(@fields)<4);
	...
	$hap_number=0;
	open(INFILE, "$mix_hap_file.haploInfo.txt")||die("Error in opening file $mix_hap_file\n");
	while(my $line = <INFILE>){
		chomp($line);
		my @fields = split(/\t/, $line);
		next if(scalar(@fields)<4);
