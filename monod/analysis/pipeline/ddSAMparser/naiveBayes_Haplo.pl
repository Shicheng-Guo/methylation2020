...
sub main{
	...
	while(my $line = <STDIN>){
		...
		if($list_calls and (length($list_calls) >= $min_length) and $haploTable{$list_pos}){
			...
			my ($count_file1, $count_file2) = (0,0);
			my ($haplo_file1, $haplo_file2) = (0,0);
			my ($count_match_file1, $count_match_file2) = (0,0);
			my ($best_match, $best_mismethyl, $best_mismatch) = ("NA", length($list_calls), length($list_calls));
			foreach my $can(shuffle @candidates){
				my ($file, $call) = split /\|/, $can;
				if($file eq 1){
					$count_file1+=$haploTable{$list_pos}->{$can};
					$haplo_file1++;
				}
				if($file eq 2){
					$count_file2+=$haploTable{$list_pos}->{$can};
					$haplo_file2++;
				}
				my ($num_mismethyl, $num_mismatch) = calcHamming($list_calls, $call);
				if($num_mismatch <= $best_mismatch){
					if($num_mismatch < $best_mismatch){
						$best_mismatch = $num_mismatch;
						$best_mismethyl = $num_mismethyl;
						$best_match = $call;
						$count_match_file1 = $haploTable{$list_pos}->{$can} if($file eq 1);
						$count_match_file2 = $haploTable{$list_pos}->{$can} if($file eq 2);
					}elsif($num_mismethyl < $best_mismethyl){
						$best_mismethyl = $num_mismethyl;
						$best_match = $call;
						$count_match_file1 = $haploTable{$list_pos}->{$can} if($file eq 1);
						$count_match_file2 = $haploTable{$list_pos}->{$can} if($file eq 2);
					}else{
						# same scores, combine them as one.
						$count_match_file1 += $haploTable{$list_pos}->{$can} if($file eq 1);
						$count_match_file2 += $haploTable{$list_pos}->{$can} if($file eq 2);	
					}
				}			
			}
			# reject count if best match found is more than 10% different from reference haplotypes
			# analogy: mispelt words
			# reject count if both reference do not match the haplotype
			if( ($best_mismatch*length($best_match) < $rejection_cutoff) and 
			    ($count_match_file1 + $count_match_file2 > 0) and 
			    ($count_file1 > 0 and $count_file2 > 0)){
				# calculate the log likelihood of word in both document class
				my $denom = $count_file1 + $count_file2 + $haplo_file1 + $haplo_file2;
				$score_file1 += log(($count_match_file1+1)/($count_file1+$haplo_file1)) + log(($count_file1+1)/$denom);
				$score_file2 += log(($count_match_file2+1)/($count_file2+$haplo_file2)) + log(($count_file2+1)/$denom);
				$file1_count++ if($score_file1 > $score_file2);
				$file2_count++ if($score_file2 > $score_file1);
			}
		}
		$logp_for_1+=$score_file1;
		$logp_for_2+=$score_file2;
		undef(@ordered_pos);
		undef(%indices);
		undef(%alignment);	
	}
...
sub calcHamming{
	#string length must be equal
	my ($k, $l) = @_;
	my $len = length($k);
	my $num_mismatch = 0;
	my $num_mismethyl = 0;
	for(my $i=0; $i<$len; $i++){
		$num_mismethyl++ if substr($k, $i, 1) ne substr($l, $i, 1) and substr($k, $i, 1) eq "M";
		$num_mismatch++ if substr($k, $i, 1) ne substr($l, $i, 1);
	}
	return ($num_mismethyl, $num_mismatch);
}
...
sub printUsage{
	print " Usage: \n";
	print " ./naiveBayes_Haplo.pl [HaploInfo1] [HaploInfo2] < fillmd.sam\n";
	exit 0;
}
