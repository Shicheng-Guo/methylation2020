...
sub main{
	...
	print OUT "indices:\tAlleleCounts\tTotalReads\n";
	my $last_pos;
	my $last_chr;
	while(my $line = <STDIN>){
		chomp($line);
		...
		#print $alignment{"ref_match_qual"}, "\n";
		if(!$last_pos){
			# don't do anything
		}elsif($alignment{"pos"} > $last_pos || $last_chr ne $alignment{"chr"}){
			# current alignmnet don't have any overlap
			# print old table & clear mem
			foreach my $value(keys %allelesTable){
				next if($allelesTable{$value}->{"total_num"} < $minCoverage);
				print OUT $value, "\t";
				for(my $i = 0; $i<scalar(@alleles); $i++){
					my $count = $allelesTable{$value}->{$alleles[$i]};
					next if(!$count);
					print OUT $count, $alleles[$i], ",";
				}
				print OUT "\t", $allelesTable{$value}->{"total_num"}, "\n";
			}
			undef(%allelesTable);
		}
		$last_pos = $alignment{"pos"} + $alignment{"tlen"};
		...
		for(my $i = 0; $i <= scalar(@ordered_pos)-$locus_num_cg; $i++){
			...
			$value_call =~ tr/T/U/;
			$value_call =~ tr/C/M/;
			$allelesTable{$value_key}->{$value_call}++;
			$allelesTable{$value_key}->{"total_num"}++;
		}
		undef(@ordered_pos);
		undef(%indices);
		undef(%alignment);	
...
sub printBEDTable{
	...
	foreach my $chr_pos (keys %methylTable){
		...
		next if($total_depth < $minCoverage);
...
sub printUsage{
	print " Usage: \n";
	print " ./cgLocusAlleles.pl [out_put_name] [cpg position list] [locus_num_cg] < fillmd.sam\n";
	exit 0;
}
