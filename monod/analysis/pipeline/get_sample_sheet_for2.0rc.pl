...
# INPUT
# lane#	project	i7_file i5_file id_file
	...
	if($tmp[4]){
		...
		foreach my $i7 (keys %i7_values){
			foreach my $i5 (keys %i5_values){
				...
				my ($rcseq7, $rcseq5) = (revComp($seq7), revComp($seq5));
				print "$lane,$sname,,,,$i7,$rcseq7,$i5,$rcseq5,$project,\n";
			}
# Lane Sample_ID	Sample_Name	Sample_Plate	Sample_Well	I7_Index_ID	index	I5_Index_ID	index2	Sample_Project	Description
		}
	}
		...
		while(my $line = <IN>){
			chomp($line);
			my ($id, $seq) = split "\t", $line;
			my $rcseq = revComp($seq);
			print "$lane,$id,,,,,$rcseq,,,$project,\n";
		}
	}
