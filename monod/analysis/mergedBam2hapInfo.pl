...
foreach my $target_id (sort keys(%targetTable)){
	...
	foreach my $line (@lines){
		...
		#print "$read_id\t$CIGAR\t$read_start\t$read_strand\t$seq\n";
		foreach my $CpG_position(@CpG_positions){
			my $offset = $CpG_position-$read_start+1;
			next if($offset<0 || $offset >= length($seq)-1);	
			if($read_strand<16){
				$readHapInfo{$read_id}->{$CpG_position}=substr($seq,$offset,1);
			}else{
				$readHapInfo{$read_id}->{$CpG_position}=$rcTable{substr($seq,$offset+1,1)};				
			}	
