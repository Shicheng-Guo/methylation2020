...
for (i in 1:iteration){
  command=paste("bedtools shuffle","-i",bed1,"-g",chrom.size,"| bedtools intersect -wa -u -a",bed2,"-b - | wc -l >> output2",sep=" ")
  try(system(command))
  print(i)
}
...
rlt<-bedEnrichmentTest(bed1="rrbs.encode.mb.0.6.bed",bed2="../phase3/WGBS_pooled_mappable_bins.mld_blocks_r2-0.5.bed",iteration=1000000)
save(rlt,file="rrbs.wgbs.overlap.RData")

setwd("/home/sguo/monod/tcga")
rlt<-bedEnrichmentTest(bed1="Conservative.methylation.block.0.6.bed.txt",bed2="WGBS_pooled_mappable_bins.mld_blocks_r2-0.5.bed",iteration=1000000)
save(rlt,file="tcga.wgbs.overlap.RData")

bedEnrichmentTest(bed1="Conservative.methylation.block.C11.0.6.bed.txt",bed2="Conservative.methylation.block.0.6.bed.txt",iteration=1000000)

package.skeleton(name="bedEnrichmentTest", code_files="bedEnrichTest.R")  
R CMD build bedEnrichmentTest 

command=paste("bedtools shuffle","-i",bed1,"-g","hg19.chrom.sizes","| bedtools intersect -wa -a",bed2,"-b - | wc -l",sep=" ")
try(system(command))
