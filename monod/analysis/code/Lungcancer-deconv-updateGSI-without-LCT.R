Lungcancerdatapre<-function(){
  ...
  data1<-data[,grep("Brain|Stomach|Lung|Heart|Colon|WBC|Liver|Esophagus|Kidney|LCP",colnames(data))]
...
tt<-ci95(Rlt$out.all)
colnames(tt)=colnames(Rlt$out.all)
output<-t(tt)
write.table(output,file="xx.txt",sep="\t",quote=F)
write.table(Rlt$out.all,file="Realdata-Lung-updateGSI-withoutLCT.deconvolution.txt",sep="\t",quote=F,col.names=NA,row.names=T)
save.image(file="Lung.cancer.updateGSI0.withoutLCT.01.RData")
