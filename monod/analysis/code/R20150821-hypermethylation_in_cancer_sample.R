...
# methylation variation in cancer and normal (hyper-variation in cancer samples)
# compare the variation between cancer and normal (adjusted with methylation level)
...
for(j in 1:length(file)){
  ...
  rownames(newdata)<-gsub("[ ]","",as.character(bed2cor(map[,1:3])))
  fn1<-paste(cancer,"hm450.dena.sort.RData",sep=".")
  save(newdata,file=fn1)
  
  if(nrow(map)==nrow(newdata)){
    cancerdata<-newdata[,seq(1,ncol(newdata),by=2)]
    normaldata<-newdata[,seq(2,ncol(newdata),by=2)]
    
    cancer.mean<-apply(cancerdata,1,sd)
    cancer.var<-apply(cancerdata,1,var)
    cancer.sd<-sqrt(cancer.var)
    cancer.cv<-var/cancer.mean
  
    normal.mean<-apply(normaldata,1,sd)
    normal.var<-apply(normaldata,1,var)
    normal.sd<-sqrt(normal.var)
    normal.cv<-var/normal.mean    
  
  }
  fn2<-paste(cancer,"hm450.dena.sort.variation.RData",sep=".")
  rlt<-list()
  rlt$cancer.mean<-cancer.mean
  rlt$cancer.var<-cancer.var
  rlt$cancer.sd<-cancer.sd
  rlt$cancer.cv<-cancer.cv
  
  rlt$normal.mean<-normal.mean
  rlt$normal.var<-normal.var
  rlt$normal.sd<-normal.sd
  rlt$normal.cv<-normal.cv
...
load("BRCA.hm450.dena.sort.variation.RData")
ls()
rlt$mean
rlt$var
qplot(x=rlt$mean,y=rlt$var)
library("ggplot2")
