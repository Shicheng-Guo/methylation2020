...
outputprefix="NormalPlasma-TopGSIByCategory"
...
RealMatrix<-data.frame((data1[,grep("NCP",colnames(data1))]))
...
RealMatrix=data.frame(DeconData.tmp[,grep("NCP",colnames(DeconData))])
Signatures=data.frame(DeconData.tmp[,-grep("NCP",colnames(DeconData))])
library(car)
...
acc
boxplot(acc,cex.axis=0.7,col="blue")
dev.off()
colMeans(acc)
# prior knowledge WBC contribution usually is very high (>0.1). 
acc<-acc[-(which(acc[,10]<0.1)),]
acc
boxplot(acc,cex.axis=0.7,col="blue")
dev.off()
write.table(acc,file="NormalPlasma-RealSample.Deconvolution.txt",col.names=NA,row.names=T,sep="\t")
write.table(colMeans(acc),file="NormalPlasma-RealSample.ColMeans.Deconvolution.txt",col.names=NA,row.names=T,sep="\t")
```
