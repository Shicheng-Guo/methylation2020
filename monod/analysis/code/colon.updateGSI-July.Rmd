...
Coloncancerdatapre<-function(){
  ...
  data1<-data[,grep("Brain|Stomach|Lung|Heart|Colon|WBC|Intestine|Liver|Esophagus|Kidney|CCP|CCT|CCTmix|NCP|LCTmix|LCP",colnames(data))]
...
TopGSIByCategory<-function(gsi,top=2,thresHigh=0.3,thresLow=0.1,plotfigure=T,figprefix="tissuespecific"){
  ...
  if(plotfigure==T){
    HeatMap(data=data.matrix(GSIRlt[,4:ncol(GSIRlt)]),phen=gsub("AVE.","",colnames(GSIRlt)[4:ncol(GSIRlt)]),figure=otf2)
  }
  return(GSIRlt)
}
...
# Deconvolution-QR-Real Colon Plasma
...
library("DeconRNASeq")
Rlt1<-DeconRNASeq(NewRealMatrix1,NewSignatures,checksig=FALSE,known.prop = F, use.scale = TRUE, fig = TRUE)
Rlt2<-DeconRNASeq(NewRealMatrix2,NewSignatures,checksig=FALSE,known.prop = F, use.scale = TRUE, fig = TRUE)
Rlt3<-DeconRNASeq(NewRealMatrix3,NewSignatures,checksig=FALSE,known.prop = F, use.scale = TRUE, fig = TRUE)
acc1<-Rlt1$out.all
acc2<-Rlt2$out.all
acc3<-Rlt3$out.all
...
boxplot(acc3)
colMeans(acc3)
write.table(acc1,file="Colon-RealSample.Deconvolution.txt",col.names=NA,row.names=T,sep="\t")
write.table(acc2,file="NormalPlasma-RealSample.Deconvolution.txt",col.names=NA,row.names=T,sep="\t")
write.table(acc3,file="LungPlasma-RealSample.Deconvolution.txt",col.names=NA,row.names=T,sep="\t")
write.table(colMeans(acc1),file="Colon-RealSample.ColMeans.Deconvolution.txt",col.names=NA,row.names=T,sep="\t")
write.table(colMeans(acc2),file="NormalPlasma-RealSample.ColMeans.Deconvolution.txt",col.names=NA,row.names=T,sep="\t")
write.table(colMeans(acc3),file="LungPlasma-RealSample.ColMeans.Deconvolution.txt",col.names=NA,row.names=T,sep="\t")
```
