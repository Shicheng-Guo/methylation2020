...
new<-cbind(sapply(sam[,1],function(x) unlist(strsplit(x,":"))[2]),sam[,2])
new<-data.frame(new)
refgene<-read.table("../../db/hg19/hg19.bed")
write.table(new,file="../sampleinfo.sort.new.txt",sep="\t",quote=F,col.names=NA,row.names=T)

file1<-read.table("WGBS_methHap_load_matrix_16Oct2015.txt",head=T,sep="\t",row.names=1,as.is=T,check.names=F)
file2<-read.table("WGBS_methHap_load_matrix_Oct2015.txt",head=T,sep="\t",row.names=1,as.is=T,check.names=F)
file3<-read.table("WGBS_SeqCap_methHap_load_matrix_16Oct2015.txt",head=T,sep="\t",row.names=1,as.is=T,check.names=F)
