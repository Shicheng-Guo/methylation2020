...
# CCT high while WB, NP low 
target1.colon<-which(apply(Newdata,1,function(x)  mean(x[grep("WB",colnames(Newdata))],na.rm=T)<0.1 && mean(x[grep("NCT",colnames(Newdata))],na.rm=T)<0.2 && mean(x[grep("CCT",colnames(Newdata))],na.rm=T)>0.5 && mean(x[grep("ONT",colnames(Newdata))],na.rm=T)<0.1))  # Cancer tissue > 0 and normal plasma =0
length(target1.colon)
Newdata2=Newdata[target1.colon,]
rlt<-DataSummary(Newdata2)
rlt
...
# CCT high while WB, NP low 
target1.colon<-which(apply(Newdata,1,function(x)  
  mean(x[grep("WB",colnames(Newdata))],na.rm=T)<0.1 && mean(x[grep("CCT",colnames(Newdata))],na.rm=T)>0.5))  # Cancer tissue > 0 and normal plasma =0
length(target1.colon)
Newdata2=Newdata[target1.colon,]
rlt<-DataSummary(Newdata2)
rlt
