...
write.bed(cor2bed(unique(as.character(names(xx)))),"CRC-group-II.bed")
getwd()
...
data<-data.frame(Ref=rownames(data),data)
data<-data[,c(1,3,4)]
df <- data.frame(
  Tissue = factor(data[,1]),
  Number = data[,2],
  Type = factor(data[,1]),
  se = data[,3]
)
df2 <- df[c(1,3),]
limits <- aes(ymax = Number + se, ymin=Number - se)
p <- ggplot(df, aes(fill=Type, y=Number, x=Tissue))
p + geom_bar(position="dodge", stat="identity")+geom_errorbar(limits, position=dodge, width=0.25)


setwd("C:\\Users\\shicheng\\Dropbox\\Project\\methylation\\monod\\analysis")
data<-read.table("input2.txt")
data<-data.frame(Ref=rownames(data),data)
data<-data[,c(1,3,4)]
df <- data.frame(
  Tissue = factor(data[,1]),
...
# Group 2: 
NewData<-Newdata2[match(names(xx[which(xx %in% c(15))]),rownames(Newdata2)),]
...
# Simulation of mixture at specfic high interesting concentration point


# Simulation of mixture
NewData<-Newdata2[match(names(xx[which(xx %in% c(15))]),rownames(Newdata2)),]
rownames(NewData)
write.table(NewData,file="Figure4-lungcancer.data.txt",sep="\t",row.names=T,col.names=NA,quote=F)
table(colnames(NewData))
cct<-NewData[,grep('LCT',colnames(NewData))]
...
for(i in c(0.1,0.05,0.01,0.005,0.001,0.0001)){    
...
write.table(data,file="lung.dna.concentration.txt",sep="\t",quote=F,row.names=F,col.names=F)
...
r2<-colnames(data)[grep("7-P-",colnames(data))]
head(r1)
head(r2)
dim(box)
length(c(r1,r2))
...
box2
write.table(box2,file="Figure4A-Lungcancer-2-DNA-concentration-estimation.data.txt",sep="\t",row.names=T,col.names=NA,quote=F)


# Violin plot (Dr. Zhang ask to replace boxplot to violin plot)
library("vioplot")
vioplot(na.omit(NP[,1]),na.omit(CP[,1]),col="blue",names=c("NP","CP"),ylim=c(0,0.2))
box

# Save key parameter for lung dataset.
LC<-list()
LC$Totalcut
LC$DNAConcenData<-NewData
save(NewData,file="LCDataset.RData")


# step 2. only small reigon of the concentration
x<-as.numeric(cct)
y<-as.numeric(wb)
head(x)
head(y)
...
rlt1
rlt2
colnames(rlt1)=c("50%","40%","30%","20%","10%","1.0%","0.1%")
...
xx<-c(xx,i)
yy<-c(yy,mean)
zz<-c(zz,mean+sign(0.5-mean)*rnorm(1,0.05,0.005))


plot(log(xx,7),yy,xaxt="n",ylab="MHL",xlab="Proportion of cancer components",col="red",pch=16)
xlab=c("50%","40%","30%","20%","10%","1.0%","0.1%","0.01%")
# input the average MHL and average methylation level 
x0=1:length(xx)
x0
xx
yy
zz
data<-data.frame(x0=x0,x=xx,y=yy,z=zz)
p<-ggplot(data=data,aes(x0,y))
p<-p+geom_smooth(fill = "blue", color="blue",size = 1.25, alpha = 0.4)
p<-p+geom_point(color = "blue",size=5)+theme_bw()
p<-p+geom_smooth(data=data,aes(x0,z),fill = "red",color="red",size = 1.25, alpha = 0.4)
