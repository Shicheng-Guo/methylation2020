...
colnames(data)
target1.colon<-which(apply(data,1,function(x) sum(x[31:35]>0.35)>=3 && sum(x[36:55]>0.03)<=5))
...
for(i in 1:length(target1.colon)){
  ...
  ct<-c(ct,mean(as.numeric(data[i,31:35])))
...
t.test(dataSummary[1:4],dataSummary[9:12])
...
myData
myData$type <- factor(myData$type, levels = c("CP","CT","NP"))
myData
...
colnames(data)
target1.lung<-which(apply(data,1,function(x) sum(x[30:34]>0.32)>=3 && sum(x[35:54]>0.02)<=8))
length(target1.lung)
data=data[target1.lung,]
cp<-lt<-np<-c()
for(i in 1:length(target1.lung)){
  cp<-c(cp,mean(as.numeric(data[i,1:29])))
  lt<-c(lt,mean(as.numeric(data[i,30:34])))
...
dataSummary<-data.frame(dp2,type)
head(dataSummary)
t.test(dataSummary[1:5,1],dataSummary[11:15,1])
...
colnames(data)
data
target1.pancreatic<-which(apply(data,1,function(x) sum(x[11:15]>0.3)>1 && sum(x[16:35]>0.01)<=4))
length(target1.pancreatic)
data=data[target1.pancreatic,]
cp<-tp<-np<-c()
for(i in 1:length(target1.pancreatic)){
...
head(dp)
dp2<-as.numeric(dp)
type<-c(rep("CP",length(dp[,1])),rep("PT",length(dp[,1])),rep("NP",length(dp[,1])))
dataSummary<-data.frame(dp2,type)
head(dataSummary)
t.test(dataSummary[1:4,1],dataSummary[9:12,1])
myData <- aggregate(dataSummary$dp2,by =list(type=dataSummary$type),
                    FUN = function(x) c(mean = mean(x), sd = sd(x),
                                        n =as.numeric(length(x)),
                                        sem=sd(x)/sqrt(n),
...
colnames(myData)=c("type","mean","sd","n","sem","me")
myData
myData$type <- factor(myData$type, levels = c("CP","PT","NP"))
myData
