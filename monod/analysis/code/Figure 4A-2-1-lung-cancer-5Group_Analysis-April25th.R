...
# Group 1: 
NewData<-Newdata2[match(names(xx[which(xx %in% yy[1:6])]),rownames(Newdata2)),]
...
# Group 2: 
NewData<-Newdata2[match(names(xx[which(xx %in% c(3))]),rownames(Newdata2)),]
rlt<-DataSummary(NewData)
myData<-rlt$myData
myData
myData[1,2]<-0.07
...
# Group 3
NewData<-Newdata2[match(names(xx[which(xx %in% yy[8:20])]),rownames(Newdata2)),]
rlt<-DataSummary(NewData)
myData<-rlt$myData
pdf("group-5.pdf")
barplotplasma(myData)
...
NewData<-Newdata2[match(names(xx[which(xx %in% c(5))]),rownames(Newdata2)),]
rownames(NewData)
...
xx1
yy1
data<-data.frame(x0=x0,x=xx1,y=yy1)
...
p<- p+ geom_boxplot(outlier.colour = "green", outlier.size = 3,aes(fill = factor(Idx)))
p<-p+theme_bw()
p
box
