...
d1<-read.table("LAD.hg18.bed",as.is=T,sep="\t")
d2<-read.table("LAD.hg19.bed",as.is=T,sep="\t")
d3<-read.table("LAD.hg38.bed",as.is=T,sep="\t")
head(d1)
head(d2)
head(d3)
...
write.table(bed2,file="LAD.hg19.bed4",col.names=F,row.names=F,quote=F,sep="\t")
write.table(bed3,file="LAD.hg38.bed4",col.names=F,row.names=F,quote=F,sep="\t")
