...
yy1<-c()
zz1<-c()
# Analysis-2: This is only used to show the important point of the relationship between mean(MHL) and F(CancerDNA)
...
write.table(data,file="pancreatic.dna.concentration.txt",sep="\t",quote=F)
...
# Analysis-1: extrapolate cancer dna fragement fra (2 or 10 as base)
library("ggplot2")
xx1<-c()
yy1<-c()
zz1<-c()
...
r2
box
write.table(box2,file="Figure4A-Pancreatic-DNA-concentration-estimation.data.txt",sep="\t",row.names=T,col.names=NA,quote=F)
