...
write.bed(cor2bed(unique(as.character(names(xx)))),"LC-group-II.bed")
getwd()
...
xlab=xx1
dataP<-data.frame(x=xx1,y=yy1,z=zz1)
dataP$x0=1:length(xx1)
x0=1:length(xx1)
xx1
yy1
dataP<-data.frame(x0=x0,x=xx1,y=yy1,z=zz1)

write.table(dataP,file="colon.dna.concentration.txt",sep="\t",quote=F)

p<-ggplot(data=dataP,aes(x,y))
p<-p+geom_smooth(fill = "blue", color="blue",size = 1.25, alpha = 0.4)
p<-p+geom_point(color = "blue",size=5)+theme_bw()
p<-p+geom_smooth(data=dataP,aes(x,z),fill = "red",color="red",size = 1.25, alpha = 0.4)
p<-p+geom_point(data=dataP,aes(x,z),color = "red",size=5)
p<-p+theme_bw()
...
# estimate cancer dna proportion based on MHL( y -> x)
  stdcurve<-data.frame(cen=xx1,mhl=yy1)
  write.table(stdcurve,file="colon.std.curve.txt",col.names=NA,row.names=T,sep="\t",quote=F)
  NewData<-Newdata2[match(names(xx[which(xx %in% c(1))]),rownames(Newdata2)),]
...
# Estimate cfDNA fragment for paired tissue and plasma samples
stdcurve<-data.frame(cen=xx1,mhl=yy1)
write.table(stdcurve,file="colon.std.curve.txt",col.names=NA,row.names=T,sep="\t",quote=F)
NewData<-Newdata2[match(names(xx[which(xx %in% c(1))]),rownames(Newdata2)),]

G1<-grep("NP-Kun",colnames(NewData))
G2<-grep("CCP",colnames(NewData))
...
findConcentration<-function(x,xx,yy){
  rlt<-xx[length(xx)-round(abs(rnorm(1,3,3)))]; # detection limitation (0.001%)
  for(i in 2:length(yy)){
    if(x<yy[i-1] & x>yy[i]){
      rlt<-(yy[i]-yy[i-1])/(yy[i]-yy[i-1])*(xx[i-1]-xx[i])+xx[i-1]
      break
    }
  }
  return(rlt)
...
rlt1<-lapply(GG1,function(x) findConcentration(x,xx1,yy1))
rlt2<-lapply(GG2,function(x) findConcentration(x,xx1,yy1))

unlist(rlt1)
unlist(rlt2)

write.table(t(data.frame(rlt1)),file="NP.cfDNA.estiamtion.rlt.txt",col.names=NA,quote=F,sep="\t")
write.table(t(data.frame(rlt2)),file="CRC-plasma.cfDNA.estiamtion.rlt.txt",col.names=NA,quote=F,sep="\t")


# Estimate cfDNA fragment for all normal plasma samples
stdcurve<-data.frame(cen=xx1,mhl=yy1)
write.table(stdcurve,file="colon.std.curve.txt",col.names=NA,row.names=T,sep="\t",quote=F)
NewData<-Newdata2[match(names(xx[which(xx %in% c(1))]),rownames(Newdata2)),]
