...
sig<-subset(Rlt,P.FDR<0.05 & (as.numeric(as.character(Delta))< -0.1|as.numeric(as.character(Delta))>0.5))
dim(sig)
write.table(sig,file="pancreatic.cancer.plasma.vs.normal.plasma.significant.txt",col.names=NA,row.names=T,sep="\t",quote=F)
...
colnames(tmp)
table(colnames(tmp))
colnames(tmp)<-c(rep("PCP",length(grep("PCP",colnames(tmp)))),rep("NP",length(grep("NP-Kun",colnames(tmp)))))
...
pdf("Supplementary.pancreatic.sig.all.heatmap.pdf")
