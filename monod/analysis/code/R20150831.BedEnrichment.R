###################################################################################
###   Title: Bed enrichment analysis to comprehensive reference database. 
###   Author: Shicheng Guo, Ph.D. Email: Shicheng.Guo@hotmail.com 
###   bedEnrichment analysis between two bed files. Do A enriched in B and then reverse. 
...
# ref=list.files(path="~/db/hg19/",pattern="*hg19.bed")
# ref=list.files(path="~/db/hg19/",pattern="*bed$")
 ref=list.files(path="~/db/hg19/",pattern="^Super*")
...
for(i in ref){
  # bed1="WGBS_pooled_mappable_bins.mld_blocks_r2-0.5.bed"
  bed1="wgbs.bed"
  bed2=paste("~/db/hg19/",i,sep="")
  excl="~/db/hg19/CRGmapability.hg19.exclude.bed"
  assembly="hg19"
...
write.table(out,file="WGBS_pooled_mappable_bins.bedenrichment.result.partA.txt",sep="\t",quote=F,row.names=F,col.names=T)
...
for(i in ref){
  bed2="/home/sguo/monod/phase3/WGBS_pooled_mappable_bins.mld_blocks_r2-0.5.bed"
  bed1=paste("/home/sguo/annotation/hg19/",i,sep="")
  excl="/home/sguo/annotation/hg19/CRGmapability.hg19.exclude.bed"
  assembly="hg19"
  nbed1=read.table(bed1)
  nbed2=read.table(bed2)
  rlt<-bedEnrichmentTest(bed1=bed1,bed2=bed2,excl=excl,iteration=500,assembly)
  observation=c(observation,rlt$observation)
  expectation=c(expectation,mean(rlt$expectation))
  fc=c(fc,rlt$observation/mean(rlt$expectation))
  ratio=c(ratio,rlt$observation/nrow(nbed1))
...
##### load the below function in advance

bedEnrichmentTest<-function(bed1,bed2,excl,assembly="hg19",iteration=1000){
  rlt<-list()
  ...
  try(system(command),silent=T)
  count1<-read.table("output1",as.is=T)
  observe<-nrow(count1)
  try(system(paste("rm","output1",sep=" " )),silent=T)
  try(system(paste("rm","output2",sep=" " )),silent=T)
  chrom.size=paste(assembly,"chrom.sizes",sep=".")
  for (i in 1:iteration){
    command=paste("bedtools shuffle","-i",bed1,"-excl",excl,"-g",chrom.size,"| bedtools intersect -wa -u -a -","-b",bed2,"|wc -l >> output2",sep=" ")
    try(system(command),silent=T)
    print(i)
  }
  count2<-read.table("output2",as.is=T)
  
  commandA=paste("wc -l",bed1,sep=" ")
  commandB=paste("wc -l",bed2,sep=" ")
  
  print(paste("Excluded regions in genomic shuffling is",excl,sep=" "))
  ...
  if(mean(count2[,1])<observe){
    pvalue<-sum(count2[,1] >= observe)/iteration
  }else{
    pvalue<-1-sum(observe<=count2[,1])/iteration
  }
  ...
  try(system("mv output2 expected.overlab.counts.number"),silent=T)
