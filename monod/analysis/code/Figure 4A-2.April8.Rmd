...
Group[grep("STL|N37-|methylC-|WB|NLT|NPT",Group)]<-"HNT"
# Group[grep("STL|N37-|methylC-|WB|NCT|NPT",Group)]<-"HNT"
# Group[grep("STL|N37-|methylC-|WB|NCT|NLT",Group)]<-"HNT"
newdata<-data
...
delta1=-2
for(i in seq(0.01,0.2,by=0.02)){
  for(j in seq(0.01,0.4,by=0.05)){
    for(x in seq(0.01,0.4,by=0.05)){
      
      target1.colon<-which(apply(Newdata,1,function(x) mean(x[grep("HNT",colnames(Newdata))],na.rm=T)< i && mean(x[grep("NCT",colnames(Newdata))],na.rm=T)> j && mean(x[grep("CCT",colnames(Newdata))],na.rm=T)>x && mean(x[grep("CCP",colnames(Newdata))],na.rm=T)>0))  # Cancer tissue > 0 and normal plasma =0
      ...
      myData <- do.call(data.frame, myData)
      delta=as.numeric(as.character(myData[1,2]))-as.numeric(as.character(myData[5,2]))
      if(delta>delta1){
        delta1=delta
        print(c(i,j,x,delta))
      }
...
target1.colon<-which(apply(Newdata,1,function(x) mean(x[grep("HNT",colnames(Newdata))],na.rm=T)<0.01 && mean(x[grep("NCT",colnames(Newdata))],na.rm=T)>0.3 && mean(x[grep("CCT",colnames(Newdata))],na.rm=T)>0.3 && mean(x[grep("CCP",colnames(Newdata))],na.rm=T)>0))  # Cancer tissue > 0 and normal plasma =0
...
myData <- within(myData,type <- factor(type,levels=c("NP-Kun","HNT","NCT","CCT","CCP")))
...
# YY<-grep("NP|CCP|CCT|NCT",Group)
# YY<-grep("NP-Kun|CCP|CCT|NCT",Group)
YY<-grep("NP-Kun|LCP|LCT|HNT",Group)
Newdata<-newdata[,YY]
colnames(Newdata)
...
colnames(Newdata)
dim(Newdata)
Newdata<-Newdata[,c(grep("HNT",colnames(Newdata)),grep("NP-Kun",colnames(Newdata)),grep("LCT",colnames(Newdata)),grep("LCP",colnames(Newdata)))]
colnames(Newdata)
dim(Newdata)
target1.colon<-which(apply(Newdata,1,function(x) mean(x[grep("HNT",colnames(Newdata))],na.rm=T)<0.1 && mean(x[grep("LCT",colnames(Newdata))],na.rm=T)>0.2))  # Cancer tissue > 0 and normal plasma =0
length(target1.colon)
Newdata2=Newdata[target1.colon,]
...
colnames(colonData)
NewData<-data.frame(value=as.numeric(colonData),Group=rep(colnames(colonData),each=nrow(colonData)))
t.test(NewData[which(NewData$Group=="LCP"),1],NewData[which(NewData$Group=="NP-Kun"),1],na.rm=T)
...
xlab="Group"
title="Average MHL in different Groups"
myData <- within(myData,type <- factor(type,levels=c("NP-Kun","HNT","LCT","LCP")))
...
# YY<-grep("NP|CCP|CCT|NCT",Group)
# YY<-grep("NP-Kun|CCP|CCT|NCT",Group)
YY<-grep("NP-Kun|PCP|PCT|HNT",Group)
Newdata<-newdata[,YY]
colnames(Newdata)
...
colnames(Newdata)
dim(Newdata)
Newdata<-Newdata[,c(grep("HNT",colnames(Newdata)),grep("NP-Kun",colnames(Newdata)),grep("PCT",colnames(Newdata)),grep("PCP",colnames(Newdata)))]
colnames(Newdata)
dim(Newdata)
target1.colon<-which(apply(Newdata,1,function(x) mean(x[grep("HNT",colnames(Newdata))],na.rm=T)<0.05 && mean(x[grep("PCT",colnames(Newdata))],na.rm=T)>0.2))  # Cancer tissue > 0 and normal plasma =0
length(target1.colon)
Newdata2=Newdata[target1.colon,]
# write.table(NewData,file="Lung.cancer.tissue.hyper.txt",col.names=NA,row.names=T,quote=F,sep="\t")
...
colnames(colonData)
NewData<-data.frame(value=as.numeric(colonData),Group=rep(colnames(colonData),each=nrow(colonData)))
t.test(NewData[which(NewData$Group=="PCP"),1],NewData[which(NewData$Group=="NP-Kun"),1],na.rm=T)
...
xlab="Group"
title="Average MHL in different Groups"
myData <- within(myData,type <- factor(type,levels=c("NP-Kun","HNT","PCT","PCP")))
