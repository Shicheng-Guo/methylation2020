Coloncancerdatapre<-function(){
  ...
  data1<-data[,grep("Brain|Stomach|Lung|Heart|Colon|WBC|Intestine|Liver|Esophagus|Kidney|LCP|LCT|LCTmix",colnames(data))]
  data1_ref<-data1[,-(grep("LCP|LCTmix",colnames(data1)))]  
...
outputprefix="Lungcancer.plasma.0.05.top100"
library("DeconRNASeq")
library("ggplot2")
Colon<-Coloncancerdatapre()
save(Colon,file=paste(outputprefix,".input.RData",sep=""))
load("Lungcancer.plasma.0.05.top100.input.RData")
...
load("Lungcancer.plasma.0.05.top100.input.RData")
data=Colon$data
cctMix=data[,grep("LCTmix",colnames(data))]
...
outputprefix="Lung-TopGSIByCategory"
topgsi<-TopGSIByCategory(Colon$GSI,top=100,thresHigh=0.3,thresLow=0.1,plotfigure=T,figprefix=outputprefix)

# simulation data and real data
VirtualMatrix<-data.frame((data1[,grep("LCTmix",colnames(data1))]))
RealMatrix<-data.frame((data1[,grep("LCP",colnames(data1))]))
...
library("DeconRNASeq")
Rlt<-DeconRNASeq(NewVirtualMatrix,NewSignatures,checksig=FALSE,known.prop = F, use.scale = T, fig = TRUE)
acc<-Rlt$out.all
rownames(acc)=colnames(NewVirtualMatrix)
acc
write.table(acc,file="Lung-mixtureSimulation.Deconvolution.txt",col.names=NA,row.names=T,sep="\t")
...
DeconData.tmp<-DeconData[na.omit(match(topgsi[,1],rownames(DeconData))),]
RealMatrix=data.frame(DeconData.tmp[,grep("LCP",colnames(DeconData))])
Signatures=data.frame(DeconData.tmp[,-grep("LCP",colnames(DeconData))])
library(car)
...
acc
colMeans(acc)
write.table(acc,file="Lung-RealSample.Deconvolution.txt",col.names=NA,row.names=T,sep="\t")
