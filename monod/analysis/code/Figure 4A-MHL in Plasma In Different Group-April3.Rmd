...
Group[grep("STL|N37-|methylC-|NLT|NPT",Group)]<-"HNT"
...
YY<-grep("NP-Kun|CCP|CCT|NCT|HNT|WB",Group)
Newdata<-newdata[,YY]
colnames(Newdata)
...
colnames(Newdata)
dim(Newdata)
Newdata<-Newdata[,c(grep("WB",colnames(Newdata)),grep("HNT",colnames(Newdata)),grep("NCT",colnames(Newdata)),grep("NP-Kun",colnames(Newdata)),grep("CCT",colnames(Newdata)),grep("CCP",colnames(Newdata)))]
colnames(Newdata)
table(colnames(Newdata))
dim(Newdata)

target1.colon<-which(apply(Newdata,1,function(x) mean(x[grep("HNT",colnames(Newdata))],na.rm=T)<0.1 && mean(x[grep("CCT",colnames(Newdata))],na.rm=T)>0.3 && mean(x[grep("NP",colnames(Newdata))],na.rm=T)))  # Cancer tissue > 0 and normal plasma =0
...
ggplot(myData, aes(x =type, y = mean)) +  
  geom_bar(position = position_dodge(), stat="identity", fill="blue",width=0.9) + 
  geom_errorbar(aes(ymin=mean-2*sem, ymax=mean+2*sem),size=1.0) +
  ggtitle(title) + 
  theme_bw() +
  theme(panel.grid.major = element_blank())+
  xlab(xlab) +
