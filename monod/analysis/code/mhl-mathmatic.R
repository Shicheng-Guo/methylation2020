...
mhlA<-mhl(hap_comb(unique(hapinfo)))
# Figuure 2B
hapinfo<-c(rep("CCCC",16))
mhlB<-mhl(hap_comb(unique(hapinfo)))
# Figuure 2C
hapinfo<-c(rep("CCCC",8),rep("TTTT",8))
mhlC<-mhl(hap_comb(unique(hapinfo)))
# Figuure 2D
hapinfo<-c(rep("CCCC",8),rep("TTTT",8))
mhlC<-mhl(hap_comb(unique(hapinfo)))
# Figuure 2E
hapinfo<-c(rep("CCTT",8),rep("TTCC",8))
mhlE<-mhl(hap_comb((hapinfo)))
...
mfRlt
plot(mhlRlt,type="n")
lines(mhlRlt,type="p",col="red")
lines(mfRlt,type="p",col="blue")
