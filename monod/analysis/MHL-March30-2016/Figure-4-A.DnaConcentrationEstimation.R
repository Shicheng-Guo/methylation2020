...
write.table(data,file="colon.dna.concentration.txt",sep="\t",quote=F)
...
p<-p+geom_smooth(data=data,aes(x,z),fill = "red",color="red",size = 1.25, alpha = 0.4)
p<-p+geom_point(data=data,aes(x,z),color = "red",size=5)
p<-p+theme_bw()
...
r2<-colnames(data)[grep("6-P",colnames(data))]
