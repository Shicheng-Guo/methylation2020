...
colnames(Newdata)
dim(Newdata)
target1.colon<-which(apply(Newdata,1,function(x) mean(x[grep("NP-Kun",colnames(Newdata))],na.rm=T)<0.1 && mean(x[grep("LCT",colnames(Newdata))],na.rm=T)>0.3))  # Cancer tissue > 0 and normal plasma =0
...
colnames(Newdata)
dim(Newdata)
target1.colon<-which(apply(Newdata,1,function(x) mean(x[grep("NP-Kun",colnames(Newdata))],na.rm=T)<0.05 && mean(x[grep("PCT",colnames(Newdata))],na.rm=T)>0.4))  # Cancer tissue > 0 and normal plasma =0
