...
# Annotation 
  ...
  target.txt
  data1<-read.table("target.txt",head=T,sep="\t")
  data2<-read.table("phase21.bed",head=F,sep="\t")
  data2<-data2[-which(data2[,22] %in% data1[,1]),]
  write.table(data2,file="phase2.txt",quote=F,sep="\t",col.names=NA,row.names=T)
