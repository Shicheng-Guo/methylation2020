...
library("RISmed")
fit <- EUtilsSummary("shox2[Title/Abstract]+methylation[Title/Abstract]", db = "pubmed")
...
############################################################
#############   Meta-Analysis to Mean Difference ###########
############################################################
library("meta")
data("amlodipine")
amlodipine
...
rlt<-rma(yi,vi,method="FE", data=dat)
rlt<-rma(yi,vi, data=dat)
plot(rlt)
