...
for(n in 1:N){
  ...
  for(i in 1:K){
    ...
    rlt1[i,]<-assess(model,train,test,trainlabel,testlabel)
  }
  rltt1[n,]<-colMeans(rlt1,na.rm=T)
}
rltt1
write.table(rltt1,file="logistic.accuracy.result.txt",sep="\t",quote=F,row.names=T,col.names=NA)
