...
#####################################################################################################
library("ggplot2")
setwd("//mcrfnas2/bigdata/Genetic/Projects/shg047/methylation/chol")
...
for(i in 1:length(GeneSymbol)){
  ...
  Mean<-tapply(newdata,phen,function(x) mean(x,na.rm=T))
...
Table = data.frame(OR,LowerLimit=CI.lower,UpperLimit=CI.upper,Group,Normal,Cancer)
write.table(Table,file="CHOR.Merge.rlt.txt",sep="\t",quote=F,col.names = NA,row.names =T)
dim(Table)
...
####################### 2019-02-23  ###############################
###########################################################################################################
# merge phase I and phase II dataset and prepare the manuscript
...
mydata<-list()
mydata$methdata=methdata
mydata$sampletype<-phen
mydata$genomeposition<-GenomePosition
mydata$genesymbol<-genesymbol

length(GenomePosition)
length(genesymbol)
length(phen)
length(genesymbol)

# wide format to long format
newdata<-data.frame(GeneSymbol=genesymbol,GenomePosition=mydata$genomeposition,methdata,check.names=F)
head(newdata)
newdataLong<-gather(newdata,"id", "MF", gathercols=colnames(methdata))
head(newdataLong)
newdataLong$type=ifelse(as.numeric(as.character(newdataLong$id)) %% 2,"Cholangiocarcinoma","Normal")
head(newdataLong)
input <- data2summary(newdataLong, varname="MF",groupnames=c("GeneSymbol","GenomePosition","type")) 
head(input)
...
for(i in unique(input$GeneSymbol)){
  ...
  inputsubset<-subset(input,GeneSymbol==i)
  head(inputsubset)
  p<- ggplot(inputsubset, aes(x=GenomePosition, y=MF, group=type, color=factor(type))) + 
    geom_line(size=1.5) +
    geom_point()+
    geom_errorbar(aes(ymin=MF-sem, ymax=MF+sem), width=.2,size=1.5,position=position_dodge(0.05))
  p<-p+labs(title=paste("methylation profile of",i,sep=" "), x="Position (bp)", y = "Methylation level (beta)")+
    theme_classic() +
    scale_color_manual(values=c('red','blue',"green","yellow"))+
    theme(axis.line = element_line(color = 'black'))+
...
for(i in unique(newdataLong$GeneSymbol)){
  ...
  options(scipen=10000)
  newdataLongsubset<-subset(newdataLong,GeneSymbol==i)
  Temp<-newdataLongsubset[sample(1:nrow(newdataLongsubset),300),]
  try(beeswarm(MF ~ type, data = Temp, pch = 16, xlab = "", ylab = "Methylation level (beta)",labels = unique(Temp$type), col= 1:length(unique(Temp$type))))
  legend("topright", legend = unique(Temp$type),title = "Sample", pch = 16, col = 1:length(unique(Temp$type)))
  dev.off()
}
