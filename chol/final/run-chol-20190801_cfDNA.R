...
#####################################################################################################
##########################  Profile and beeswarm plot for chol solid tissue ########################
#####################################################################################################
library("ggplot2")
setwd("/home/guosa/hpc/methylation/chol/final")
data= read_excel("methylation_cfDNA.xlsx",sheet = 2)
saminfo=read_excel("methylation_cfDNA.xlsx",sheet = 6)
data= as.data.frame(data)
saminfo= as.data.frame(saminfo)
...
sampletype=saminfo[match(colnames(methdata),saminfo$ID),2]

mydata<-list()
mydata$methdata=methdata
mydata$sampletype<-index2type(sampletype)
...
newdataLong$type=index2type(sampletype)
...
for(i in unique(input$GeneSymbol)){
  ...
  ggsave(p,file=paste("./profile/",i,".profile.cfdna.pdf",sep=""))
}
...
for(i in unique(newdataLong$GeneSymbol)){
  pdf(paste("./profile/",i,".beeswarm.cfdna.pdf",sep=""))
  options(scipen=10000)
  ...
  plot.data<-plot.data[sample(1:nrow(plot.data),400,replace=T),]
...
###########################################################################################################
######################  Generate ROC and SEN, SPE Tables, and Double P-values #############################
###########################################################################################################
setwd("/mnt/bigdata/Genetic/Projects/shg047/methylation/chol/final")
data= read_excel("methylation_cfDNA.xlsx",sheet = 2)
saminfo=read_excel("methylation_cfDNA.xlsx",sheet = 6)
data= as.data.frame(data)
saminfo= as.data.frame(saminfo)
...
head(rowname)
methdata[1:10,1:10]
phen=saminfo[match(colnames(methdata),saminfo$ID),2]
...
write.table(Table,file="chol.cfdna.pvalue.txt",sep="\t",quote=F,col.names = NA,row.names =T)

###########################################################################################################
######################  Generate ORs with forest styles  #############################
###########################################################################################################
methdata[1:5,1:5]
...
for(i in 1:length(GeneSymbol)){
  ...
  phen=saminfo[match(rownames(data),saminfo$ID),2]
...
Table = data.frame(OR,LowerLimit=CI.lower,UpperLimit=CI.upper,Group=GeneSymbol)

write.table(Table,file="CHOL.cfDNA.OR.txt",col.names = NA,row.names = T,quote=F,sep="\t")

p = ggplot(data=Table,aes(x = Group,y = OR, ymin = LowerLimit, ymax = UpperLimit))+
