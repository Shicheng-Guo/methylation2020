...
setwd("//mcrfnas2/bigdata/Genetic/Projects/shg047/methylation/chol/16B1212A-stDNA/profile")
data= read_excel("methylation.xlsx",sheet = 2)
data= as.data.frame(data)
rowname<-apply(data.frame(data$Target,as.character(data$GenomePosition)),1,function(x) gsub(" ","",paste(x[1],x[2],sep="")))
data[1:12,1:12]
...
setwd("//mcrfnas2/bigdata/Genetic/Projects/shg047/methylation/chol/18D0822A-ctDNA/02_Methylation_Quantification")
cfdata= read_excel("methylation.xlsx",sheet = 2)
cfdata= as.data.frame(cfdata)
rowname<-apply(data.frame(cfdata$Target,as.character(cfdata$GenomePosition)),1,function(x) gsub(" ","",paste(x[1],x[2],sep="")))
cfmethdata<-data.matrix(cfdata[,c(7:47)])
rownames(cfmethdata)<-rowname
genesymbol= unlist(lapply(cfdata$Target, function(x) strsplit(as.character(x),"_")[[1]][1]))
head(rowname)
