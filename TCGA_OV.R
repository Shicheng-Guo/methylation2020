...
followup<-unique(followup2csv(clinical.followup))
head(followup)
...
pid<-read.table("https://raw.githubusercontent.com/Shicheng-Guo/HowtoBook/master/TCGA/pid.txt")
DRUG<-c()
for(i in 1:nrow(pid)){
...
clinical.drug <- GDCprepare_clinic(query, clinical.info = "drug")
drug<-unique(drug2csv(clinical.drug))
save(drug,file=paste("TCGA",pid[i,1],"drug.RData",sep="."))
write.table(table(drug$measure_of_response),file=paste(pid[i,1],".drug.txt",sep=""),col.names = NA,row.names = T)
}

grep '[Response|Disease]'  *.drug.txt | grep -v Var | grep -v Freq > input.txt

input<-read.table("//mcrfnas2/bigdata/Genetic/Projects/shg047/methylation/OV/input.txt")


load("TCGA.TCGA-UCEC.drug.RData")
write.table(drug,file="tcga.ucec.drug.response.txt",sep="\t",col.names = NA,row.names = T,quote=F)
load("../OV/TCGA.TCGA-BRCA.drug.RData")
write.table(drug,file="tcga.brca.drug.response.txt",sep="\t",col.names = NA,row.names = T,quote=F)

pid<-read.table("https://raw.githubusercontent.com/Shicheng-Guo/HowtoBook/master/TCGA/pid.txt")
DRUG<-c()
for(i in 1:nrow(pid)){
  load(file=paste("../OV/TCGA",pid[i,1],"drug.RData",sep="."))
  write.table(drug,file=paste("tcga.",pid[i,1],".drug.txt",sep=""),col.names = T,row.names = F,quote=F,sep="\t")
}
