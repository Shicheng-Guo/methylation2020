...
# Note: STAD only have two normal tissues. 
#===========================================================
# Pair-wise DMS test
if(length(pairidv)>3){
...
#===========================================================
xtable<-table(c(which(rowMeans(data[,x1])>0.4),which(rowMeans(data[,x2])<0.2)))
...
dim(PBMCSubset)
target<-newdata[match(rownames(newdata),rownames(newdata)),]
