# Five-fold Cross-validation multiClassification.R
...
source("http://bioconductor.org/biocLite.R")
if (!require("randomForest")) biocLite("randomForest")
if (!require("stringr")) biocLite("stringr")
if (!require("impute")) biocLite("impute")
if (!require("rpart")) biocLite("rpart")
if (!require("DAAG")) biocLite("DAAG")
if (!require("PredictABEL")) biocLite("PredictABEL")
if (!require("Deducer")) biocLite("Deducer")
if (!require("Rcmdr")) biocLite("Rcmdr")
if (!require("ROCR")) biocLite("ROCR")
if (!require("MASS")) biocLite("MASS")
if (!require("e1071")) biocLite("e1071")
if (!require("BayesTree")) biocLite("BayesTree")
if (!require("class")) biocLite("class")
if (!require("randomForest")) biocLite("randomForest")
if (!require("gbm")) biocLite("gbm")
...
phen[phen=="LGG"]<-1
phen[phen=="GBM"]<-1
dat<-data.frame(phen,t(beta))

for(n in 1:N){
  index<-CvSampling(dim(dat)[1],K)
  for(i in 1:K){
    train=dat[index[[i]]$train,]
    trainlab=phen[index[[i]]$train]
    test=dat[index[[i]]$test,]
    testlab=phen[index[[i]]$test]
